...
Evaluation is a first-class workflow (`eval/evaluate.py`), run against 34 hand-written course-planning questions, with results recorded in the README and `eval/eval_results.json`.

## Capabilities and Constraints

- Retrieval and generation are separately measurable and currently measured: 34/34 (100%) retrieval hit-rate@5; answer groundedness genuinely fluctuates run to run (LLM-judge wording variance), 94-100% across this project's runs, rather than sitting at a fixed number.
...
- Scraped, parsed course data at `ingest/data/*.json`: CSCI-UA (37), MATH-UA (54), DS-UA (11), PHYS-UA (31), ECON-UA (39), PHIL-UA (48), PSYCH-UA (35), POL-UA (63) — 318 courses total across the eight scoped departments.
...
5. Scope will grow — eight CAS departments (CSCI-UA, MATH-UA, DS-UA, PHYS-UA, ECON-UA, PHIL-UA, PSYCH-UA, POL-UA; 318 courses) today, not the ceiling; CAS has 51 department pages total, and the pipeline was built (and has now been proven five times) to extend to more with minimal code change.
